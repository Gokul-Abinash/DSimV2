...
start_nodes() {
    ...
    sleep 3
    echo "SBFT nodes started on ports: ${PORTS[@]}"
...
run_tests() {
    echo "Running SBFT test transactions..."
    ...
    if [ "$show_tps" = true ]; then
        node ../tps-tester.js sbft metrics
        return
    fi
    ...
    if [ -n "$tps_count" ]; then
        if [ -n "$tps_duration" ]; then
            ...
            node ../tps-tester.js sbft duration "$tps_count" "$tps_duration"
        else
            echo "Running TPS burst test with $tps_count transactions"
            node ../tps-tester.js sbft burst "$tps_count"
        fi
        return
    fi
...
show_stats() {
    ...
    for port in "${PORTS[@]}"; do
        ...
        if lsof -i :$port >/dev/null 2>&1; then
            ...
            # Get commit count
            local commit_count=$(curl -s --max-time 5 http://localhost:$port/api/sbft-commit-log 2>/dev/null | grep -o '"committedAt"' | wc -l || echo "0")
