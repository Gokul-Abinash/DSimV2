...
# Multi-Server Distributed Prime CLI
...
start_nodes() {
    echo "=========================================="
    echo "Starting 32 Prime nodes on this machine..."
    echo "=========================================="
    ...
    for port in "${PORTS[@]}"; do
        ...
        echo "🟢 Started Prime node on port $port (PID: $pid)"
    done
...
stop_nodes() {
    echo "Stopping Prime nodes on this machine..."
    ...
    sleep 1
    echo "All local Prime nodes stopped."
}

check_status() {
    echo "=== Local Prime Nodes Status ==="
    local count=0
    for port in "${PORTS[@]}"; do
        if lsof -i :$port >/dev/null 2>&1; then
...
run_tests() {
    ...
    if [ -n "$count" ]; then
        echo "Running load test with $count transactions on Prime..."
    ...
    echo "Submitting Prime test transactions to Primary ($PRIMARY_URL)..."
...
show_stats() {
    echo "=== Local Prime Node Stats ==="
    for port in "${PORTS[@]}"; do
        if lsof -i :$port >/dev/null 2>&1; then
            local commits=$(curl -s --max-time 2 http://localhost:$port/api/prime-commit-log 2>/dev/null)
...
run_benchmark() {
    if [ -f "$SCRIPT_DIR/../benchmark-metrics.js" ]; then
        node "$SCRIPT_DIR/../benchmark-metrics.js" prime "$@"
    else
...
run_metrics() {
    if [ -f "$SCRIPT_DIR/../benchmark-metrics.js" ]; then
        node "$SCRIPT_DIR/../benchmark-metrics.js" prime evaluate "$@"
    else
        echo "Error: benchmark-metrics.js not found"
    fi
}
