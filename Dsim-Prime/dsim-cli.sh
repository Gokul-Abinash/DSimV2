...
start_nodes() {
    ...
    sleep 3
    echo "PRIME nodes started on ports: ${PORTS[@]}"
...
run_tests() {
    echo "Running PRIME test transactions..."
    ...
    if [ "$show_tps" = true ]; then
        node ../tps-tester.js prime metrics
        return
    fi
    ...
    if [ -n "$tps_count" ]; then
        if [ -n "$tps_duration" ]; then
            ...
            node ../tps-tester.js prime duration "$tps_count" "$tps_duration"
        else
            echo "Running TPS burst test with $tps_count transactions"
            node ../tps-tester.js prime burst "$tps_count"
        fi
        return
    fi
...
show_stats() {
    ...
    for port in "${PORTS[@]}"; do
        ...
        if lsof -i :$port >/dev/null 2>&1; then
            ...
            # Get commit count
            local commit_count=$(curl -s --max-time 5 http://localhost:$port/api/prime-commit-log 2>/dev/null | grep -o '"committedAt"' | wc -l || echo "0")
