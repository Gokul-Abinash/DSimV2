...
// Consensus Verification Tool for Multi-Server Distributed Prime
const fs = require('fs');
...
async function main() {
  try {
    console.log('=== Prime Cluster Consensus Verification ===');
    ...
    console.log(`Prime BFT Fault Tolerance: N=${totalN}, f=${f}, Required Quorum (2f+1)=${requiredCommitQuorum}`);
    
    if (runningNodes.length < requiredCommitQuorum) {
      console.log(`\n❌ Quorum Failure: Only ${runningNodes.length} nodes online. Need at least ${requiredCommitQuorum} for Prime BFT quorum.`);
    }
