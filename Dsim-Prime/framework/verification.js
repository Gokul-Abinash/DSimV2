...
async function collectNodeData(ports) {
  ...
  for (const port of ports) {
    try {
      const commitResponse = await axios.get(`http://localhost:${port}/api/prime-commit-log`, { timeout: 5000 });
      const logResponse = await axios.get(`http://localhost:${port}/api/prime-log`, { timeout: 5000 });
...
async function main() {
  try {
    console.log('=== Prime Consensus Verification ===');
    
    // Load test metadata
    const testData = loadTestMetadata();
    console.log(`Algorithm: Prime`);
