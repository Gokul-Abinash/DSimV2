...
console.log(`Prime Node started at ID=${myNodeID}, IP=${graph.getLocalIP()}, PORT=${PORT}`);

// View status
app.get('/', (req, res) => {
  res.send(`Node ${myNodeID}, Port ${PORT} - Prime ready`);
});
...
// Prime message endpoint
app.post('/api/prime', (req, res) => {
  protocol.handlePrimeMessage(req.body, myNodeID);
  res.json({ ok: true });
});

// Prime log viewing routes
app.get('/api/prime-log', (req, res) => {
  res.json(protocol.getPrimeNodeLog());
});

app.get('/api/prime-commit-log', (req, res) => {
  res.json(protocol.getPrimeCommitLog());
});
