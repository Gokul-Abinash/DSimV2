...
async function collectNodeData(ports) {
  ...
  for (const port of ports) {
    try {
      const commitResponse = await axios.get(`http://localhost:${port}/api/raft-commit-log`, { timeout: 5000 });
      const logResponse = await axios.get(`http://localhost:${port}/api/raft-log`, { timeout: 5000 });
...
async function main() {
  try {
    console.log('=== Raft Consensus Verification ===');
    
    const testData = loadTestMetadata();
    console.log(`Algorithm: Raft`);
