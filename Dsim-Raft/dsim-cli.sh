...
# Multi-Server Distributed Raft CLI
...
start_nodes() {
    echo "=========================================="
    echo "Starting 32 Raft nodes on this machine..."
    echo "=========================================="
    ...
    for port in "${PORTS[@]}"; do
        ...
        echo "🟢 Started Raft node on port $port (PID: $pid)"
    done
...
stop_nodes() {
    echo "Stopping Raft nodes on this machine..."
...
run_tests() {
    ...
    if [ -n "$count" ]; then
        echo "Running load test with $count transactions on Raft..."
    ...
    echo "Submitting Raft test transactions to Leader ($PRIMARY_URL)..."
...
EOF
    ...
    for val in "${VALUES[@]}"; do
        ...
        echo "TX$tx_id (value: $val) -> Leader response: $resp"
        tx_id=$((tx_id + 1))
        sleep 2
    done
...
show_stats() {
    echo "=== Local Raft Node Stats ==="
    for port in "${PORTS[@]}"; do
        if lsof -i :$port >/dev/null 2>&1; then
            local commits=$(curl -s --max-time 2 http://localhost:$port/api/raft-commit-log 2>/dev/null)
...
run_benchmark() {
    if [ -f "$SCRIPT_DIR/../benchmark-metrics.js" ]; then
        node "$SCRIPT_DIR/../benchmark-metrics.js" raft "$@"
    else
...
run_metrics() {
    if [ -f "$SCRIPT_DIR/../benchmark-metrics.js" ]; then
        node "$SCRIPT_DIR/../benchmark-metrics.js" raft evaluate "$@"
    else
        echo "Error: benchmark-metrics.js not found"
    fi
}
