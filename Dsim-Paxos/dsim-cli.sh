...
start_nodes() {
    ...
    for port in "${PORTS[@]}"; do
        ...
        echo "🟢 Started Paxos node on port $port (PID: $pid)"
    done
...
stop_nodes() {
    echo "Stopping Paxos nodes on this machine..."
    ...
    sleep 1
    echo "All local Paxos nodes stopped."
}

check_status() {
    echo "=== Local Paxos Nodes Status ==="
    local count=0
    for port in "${PORTS[@]}"; do
        if lsof -i :$port >/dev/null 2>&1; then
...
run_tests() {
    ...
    echo "Submitting Paxos test transactions to Proposer ($PRIMARY_URL)..."
...
EOF
    ...
    for val in "${VALUES[@]}"; do
        ...
        echo "TX$tx_id (value: $val) -> Proposer response: $resp"
        tx_id=$((tx_id + 1))
        sleep 2
    done
...
show_stats() {
    echo "=== Local Paxos Node Stats ==="
    for port in "${PORTS[@]}"; do
        if lsof -i :$port >/dev/null 2>&1; then
            local commits=$(curl -s --max-time 2 http://localhost:$port/api/paxos-commit-log 2>/dev/null)
