...
// Consensus Verification Tool for Multi-Server Distributed Paxos
const fs = require('fs');
...
async function collectNodeData() {
  ...
  const promises = nodes.map(async (nodeObj) => {
    ...
    try {
      const commitResponse = await axios.get(`http://${ip}:${port}/api/paxos-commit-log`, { timeout: 3000 });
      const logResponse = await axios.get(`http://${ip}:${port}/api/paxos-log`, { timeout: 3000 });
...
async function main() {
  try {
    console.log('=== Paxos Cluster Consensus Verification ===');
    ...
    console.log(`Paxos Quorum: N=${totalN}, Required Majority=Math.floor(${totalN}/2)+1 = ${majority}`);
    
    if (runningNodes.length < majority) {
      console.log(`\n❌ Quorum Failure: Only ${runningNodes.length} nodes online. Need at least ${majority} for Paxos majority.`);
    }
