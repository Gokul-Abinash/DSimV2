...
start_nodes() {
    ...
    sleep 3
    echo "MSTGHS nodes started on ports: ${PORTS[@]}"
...
EOF
    ...
    if [ "mstghs" = "mis" ]; then
        ...
        algo_name="MIS"
        payload="{}"
    elif [ "mstghs" = "mstghs" ]; then
        echo "Starting GHS MST algorithm on all nodes..."
        endpoint="api/start-ghs"
        algo_name="GHS"
        payload="{}"
    else
        echo "Starting mstghs algorithm on all nodes..."
        endpoint="api/start-mstghs"
        algo_name="mstghs"
        payload="{}"
    fi
...
show_stats() {
    echo "=== MSTGHS Cluster Statistics ==="
    ...
    for port in "${PORTS[@]}"; do
        ...
        if lsof -i :$port >/dev/null 2>&1; then
            ...
            # Get commit count
            local commit_count=$(curl -s --max-time 5 http://localhost:$port/api/mstghs-commit-log 2>/dev/null | grep -o '"committedAt"' | wc -l || echo "0")
