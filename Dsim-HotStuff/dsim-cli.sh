...
# Multi-Server Distributed HotStuff CLI
...
start_nodes() {
    echo "=========================================="
    echo "Starting 32 HotStuff nodes on this machine..."
    echo "=========================================="
    ...
    for port in "${PORTS[@]}"; do
        ...
        echo "🟢 Started HotStuff node on port $port (PID: $pid)"
    done
...
stop_nodes() {
    echo "Stopping HotStuff nodes on this machine..."
    ...
    # Clean up any leftover node processes on these ports
    for port in "${PORTS[@]}"; do
    ...
    sleep 1
    echo "All local HotStuff nodes stopped."
}

check_status() {
    echo "=== Local HotStuff Nodes Status ==="
    local count=0
    for port in "${PORTS[@]}"; do
        if lsof -i :$port >/dev/null 2>&1; then
...
run_tests() {
    ...
    echo "Submitting HotStuff test transactions to Primary ($PRIMARY_URL)..."
...
show_stats() {
    echo "=== Local HotStuff Node Stats ==="
    for port in "${PORTS[@]}"; do
        if lsof -i :$port >/dev/null 2>&1; then
            local commits=$(curl -s --max-time 2 http://localhost:$port/api/hotstuff-commit-log 2>/dev/null)
...
run_benchmark() {
    if [ -f "$SCRIPT_DIR/../benchmark-metrics.js" ]; then
        node "$SCRIPT_DIR/../benchmark-metrics.js" hotstuff "$@"
    else
...
run_metrics() {
    if [ -f "$SCRIPT_DIR/../benchmark-metrics.js" ]; then
        node "$SCRIPT_DIR/../benchmark-metrics.js" hotstuff evaluate "$@"
    else
        echo "Error: benchmark-metrics.js not found"
    fi
}
