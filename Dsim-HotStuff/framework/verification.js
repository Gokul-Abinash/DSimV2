...
async function collectNodeData(ports) {
  ...
  for (const port of ports) {
    try {
      const commitResponse = await axios.get(`http://localhost:${port}/api/hotstuff-commit-log`, { timeout: 5000 });
      const logResponse = await axios.get(`http://localhost:${port}/api/hotstuff-log`, { timeout: 5000 });
...
async function main() {
  try {
    console.log('=== HotStuff Consensus Verification ===');
    
    // Load test metadata
    const testData = loadTestMetadata();
    console.log(`Algorithm: HotStuff`);
