...
// Consensus Verification Tool for Multi-Server Distributed HotStuff
const fs = require('fs');
...
async function collectNodeData() {
  ...
  const promises = nodes.map(async (nodeObj) => {
    ...
    try {
      const commitResponse = await axios.get(`http://${ip}:${port}/api/hotstuff-commit-log`, { timeout: 3000 });
      const logResponse = await axios.get(`http://${ip}:${port}/api/hotstuff-log`, { timeout: 3000 });
...
async function main() {
  try {
    console.log('=== HotStuff Cluster Consensus Verification ===');
    ...
    const failedNodes = nodeData.filter(n => n.status === 'failed');
    ...
    console.log(`BFT Fault Tolerance: N=${totalN}, f=${f}, Required Quorum (2f+1)=${requiredCommitQuorum}`);
    
    if (runningNodes.length < requiredCommitQuorum) {
      console.log(`\n❌ Quorum Failure: Only ${runningNodes.length} nodes online. Need at least ${requiredCommitQuorum} for HotStuff BFT quorum.`);
    }
